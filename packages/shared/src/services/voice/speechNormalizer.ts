...
export class SpeechNormalizer {
  public static normalize(raw: string): NormalizedSpeechResult {
    const text = raw.trim();
    const lower = text.toLowerCase();
    
    let detectedComplaint: NormalizedSpeechResult["detectedComplaint"];
    const entities: NormalizedSpeechResult["extractedEntities"] = {};
    let confidence = 0.90;
    ...
    const assoc: string[] = [];
    if (/saans|breath|dyspnea|dum phool/i.test(lower)) assoc.push("Shortness of breath");
    if (/pasina|sweat|diaphoresis/i.test(lower)) assoc.push("Cold sweats");
    if (/ulti|vomit|nausea|ji michlana/i.test(lower)) assoc.push("Nausea / Vomiting");
    if (/chakkar|dizziness|giddiness/i.test(lower)) assoc.push("Dizziness");
