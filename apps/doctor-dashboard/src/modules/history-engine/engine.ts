import { 
  ControlledClinicalHistory, 
  ClinicalInquiryQuestion, 
  HistoryEngineState, 
  ComplaintPathway 
} from "./types";
import { resolvePathway } from "./pathways";

export class ConversationalHistoryEngine {
  private pathway: ComplaintPathway;
  private state: HistoryEngineState;

  constructor(complaintKey: string, initialHistory?: Partial<ControlledClinicalHistory>) {
    this.pathway = resolvePathway(complaintKey);
    const initial: ControlledClinicalHistory = {
  ...
  public getState(): HistoryEngineState {
    return { ...this.state };
  }

  public getHistory(): ControlledClinicalHistory {
    return { ...this.state.history };
  }

  public getCurrentQuestion(): ClinicalInquiryQuestion | null {
    return this.state.currentQuestion;
  }

  public getMissingMandatoryFields(): (keyof ControlledClinicalHistory)[] {
   ...
   */
  public answerChoice(choiceId: string): { nextQuestion: ClinicalInquiryQuestion | null; isComplete: boolean; redFlags: Array<{ condition: string; rationale: string; severity: "emergency_code_red" | "urgent_amber" }> } {
   ...
   */
  public answerNaturalText(input: string): { recognized: boolean; messageHi: string; messageEn: string } {
    const q = this.state.currentQuestion;
    if (!q) {
  ...
  private advanceQuestion(currentId: string): { nextQuestion: ClinicalInquiryQuestion | null; isComplete: boolean; redFlags: Array<{ condition: string; rationale: string; severity: "emergency_code_red" | "urgent_amber" }> } {
  ...
  private updateHistoryWithPartial(partial: Partial<ControlledClinicalHistory>) {
    for (const [key, value] of Object.entries(partial)) {
      const fieldKey = key as keyof ControlledClinicalHistory;
      if (Array.isArray(value)) {
        const existing = (this.state.history[fieldKey] as string[]) || [];
        (this.state.history as unknown as Record<string, unknown>)[fieldKey] = Array.from(new Set([...existing, ...value]));
      } else {
        (this.state.history as unknown as Record<string, unknown>)[fieldKey] = value;
      }
    }
  }
   ...
   */
  public generateStructuredSummary(): string {
    const h = this.state.history;
    const parts: string[] = [];

    parts.push(`[CHIEF COMPLAINT]: ${h.chiefComplaint}`);
    if (h.duration) parts.push(`[DURATION]: ${h.duration}`);
    if (h.onset) parts.push(`[ONSET]: ${h.onset}`);
    if (h.location) parts.push(`[LOCATION]: ${h.location}`);
    if (h.character) parts.push(`[CHARACTER]: ${h.character}`);
    if (h.radiation) parts.push(`[RADIATION]: ${h.radiation}`);
