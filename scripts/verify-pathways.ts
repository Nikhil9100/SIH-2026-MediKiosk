...
function testPathway(name: string, complaintKey: string, choicesSequence: string[], naturalInputs?: string[]) {
  console.log("\n=======================================================");
  console.log(`TESTING PATHWAY: ${name.toUpperCase()} (${complaintKey})`);
  console.log("=======================================================");

  const engine = new ConversationalHistoryEngine(complaintKey);
  let step = 1;
  ...
  if (naturalInputs && naturalInputs.length > 0) {
    ...
    const nlEngine = new ConversationalHistoryEngine(complaintKey);
...
testPathway(
  "Chest Pain (High-Risk ACS)",
  "chest_pain",
  ["dur_sudden_mins", "char_heavy_squeeze", "rad_arm_jaw", "assoc_dyspnea_sweats", "pmh_htn_dm"],
  ["2 ghante se dard hai", "kuch ajeeb sa lag raha hai"] // 1 match, 1 ambiguous
);
...
testPathway(
  "Abdominal Pain (RLQ)",
  "abdominal_pain",
  ["loc_right_lower", "dur_acute_hours", "food_aggravates", "assoc_vomiting_fever"],
  ["pet ke neeche dahini taraf", "khaana khane ke baad"]
);
...
testPathway(
  ...
  "headache",
  ["ha_thunderclap", "char_throbbing_one_side", "assoc_neuro_stiff"],
  ["achanak bijli ki tarah shuru hua", "roshni se pareshani"]
);
...
testPathway(
  "Fever (High Grade)",
  "fever",
  ["dur_1_3_days", "pat_chills_rigor", "assoc_rash_drowsy"],
  ["3 din se bukhar hai", "thand lagti hai"]
);
...
testPathway(
  ...
  "cough",
  ["type_blood_streaked", "dur_2_4w", "assoc_dyspnea_stridor"],
  ["balgam mein khoon aata hai", "saans lene mein seeti"]
);
...
testPathway(
  ...
  "back_pain",
  ["loc_lower_back", "rad_down_leg", "assoc_cauda_equina"],
  ["kamar mein dard", "peshab par control nahi"]
);
