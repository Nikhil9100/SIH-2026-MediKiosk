...
const red_flags_1 = require("../apps/kiosk-ui/src/modules/red-flags");
const auditLog_1 = require("../apps/kiosk-ui/src/modules/red-flags/auditLog");
console.log("==================================================================");
console.log("RED-FLAG DETECTION MODULE DETERMINISTIC CLINICAL TEST SUITE");
console.log("==================================================================");
function runTestCase(name, history, complaints, expectEmergency, expectedTriggerSnippet) {
    console.log("\n--- TEST CASE: " + name + " ---");
    const sessionId = "sess-test-" + Math.random().toString(36).slice(2, 7);
    const result = red_flags_1.RedFlagDetectionModule.analyze(sessionId, complaints, history, { id: "pt-test-99", name: "Ramesh Sharma", token: 77 });
...
runTestCase("1. No Red Flag (Mild Knee Aching)", {
    chiefComplaint: "Knee pain",
    duration: "2 weeks",
    severity: 3,
    associatedSymptoms: ["Mild stiffness"]
}, [{ id: "knee_joint", anatomicalRegion: "knee_joint", labelHi: "घुटने", labelEn: "Knee", severity: 3 }], false);
// 2. TEST: Single symptom (mild localized chest ache WITHOUT breathlessness, sweating, or syncope)
runTestCase("2. Single Symptom (Mild localized chest ache)", {
    chiefComplaint: "Chest pain",
    duration: "3 days",
    severity: 3,
    character: "Sharp localized",
    associatedSymptoms: ["None reported"]
}, [{ id: "chest_heart_lungs", anatomicalRegion: "chest_heart_lungs", labelHi: "छाती", labelEn: "Chest", severity: 3 }], false);
// 3. TEST: Multiple symptoms (routine multiple symptoms without emergency combo)
runTestCase("3. Multiple Symptoms (Abdominal cramps + gas + mild bloating)", {
    chiefComplaint: "Abdominal pain",
    duration: "2 days",
    severity: 4,
    associatedSymptoms: ["Dyspepsia", "Flatulence"]
}, [{ id: "stomach_abdomen", anatomicalRegion: "stomach_abdomen", labelHi: "पेट", labelEn: "Stomach", severity: 4 }], false);
// 4. TEST: Emergency combinations
// 4A. Chest pain + breathlessness
runTestCase("4A. Emergency Combo: Chest pain + breathlessness", {
    chiefComplaint: "Chest pain",
    associatedSymptoms: ["Shortness of breath / Dyspnea"]
}, [{ id: "chest_heart_lungs", anatomicalRegion: "chest_heart_lungs", labelHi: "छाती", labelEn: "Chest", severity: 7 }], true, "Chest Pain with Respiratory Compromise");
// 4B. Chest pain + severe sweating (diaphoresis)
runTestCase("4B. Emergency Combo: Chest pain + severe cold sweats", {
    chiefComplaint: "Chest pain",
    associatedSymptoms: ["Cold sweats / Diaphoresis"]
}, [{ id: "chest_heart_lungs", anatomicalRegion: "chest_heart_lungs", labelHi: "छाती", labelEn: "Chest", severity: 8 }], true, "Chest Pain with Diaphoresis");
// 4C. Chest pain + fainting (syncope)
runTestCase("4C. Emergency Combo: Chest pain + fainting", {
    chiefComplaint: "Chest pain",
    associatedSymptoms: ["Fainting / Syncope"]
}, [{ id: "chest_heart_lungs", anatomicalRegion: "chest_heart_lungs", labelHi: "छाती", labelEn: "Chest", severity: 9 }], true, "Chest Pain with Syncope");
// 4D. Sudden weakness + speech difficulty
runTestCase("4D. Emergency Combo: Sudden weakness + speech difficulty", {
    chiefComplaint: "Headache",
    associatedSymptoms: ["Sudden weakness in arm", "Speech difficulty"]
}, [{ id: "head_brain", anatomicalRegion: "head_brain", labelHi: "सिर", labelEn: "Head", severity: 6 }], true, "Sudden Focal Neurological Symptoms");
// 4E. Severe abdominal pain + fainting
runTestCase("4E. Emergency Combo: Severe abdominal pain (sev 9) + fainting", {
    chiefComplaint: "Abdominal pain",
    severity: 9,
    associatedSymptoms: ["Fainting / Collapse"]
}, [{ id: "stomach_abdomen", anatomicalRegion: "stomach_abdomen", labelHi: "पेट", labelEn: "Stomach", severity: 9 }], true, "Severe Abdominal Pain with Syncope");
// 4F. Severe breathing difficulty / Stridor
runTestCase("4F. Emergency Combo: Severe acute respiratory distress / Stridor", {
    chiefComplaint: "Cough and breathing difficulty",
    associatedSymptoms: ["Severe dyspnea / Stridor"]
}, [{ id: "throat_neck", anatomicalRegion: "throat_neck", labelHi: "गला", labelEn: "Throat", severity: 8 }], true, "Severe Acute Respiratory Distress");
// 5. TEST: False / Ambiguous input
runTestCase("5. False / Ambiguous input (Vague fatigue / tiredness)", {
    chiefComplaint: "General feeling unwell",
    associatedSymptoms: ["Tiredness", "Stress"]
}, [{ id: "fever_vitals", anatomicalRegion: "fever_vitals", labelHi: "बुखार", labelEn: "Fever", severity: 2 }], false);
// 6. TEST: Audit Log Verification
console.log("\n--- TEST CASE: 6. Audit Log Verification ---");
const logs = auditLog_1.RedFlagAuditLogService.getLogs();
...
const ackSuccess = auditLog_1.RedFlagAuditLogService.acknowledgeAlert(latestLog.auditId, "Dr. Priya Sharma (Triage Lead)");
